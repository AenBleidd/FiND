...
std::string vina_seed_remark(int seed) {
    std::ostringstream remark;
    remark << "REMARK VINA SEED: " << seed << '\n';
    return remark.str();
}
...
void do_search(model& m, const boost::optional<model>& ref, const scoring_function& sf, const precalculate& prec, const igrid& ig, const precalculate& prec_widened, const igrid& ig_widened, non_cache& nc, // nc.slope is changed
               ...
               int seed, int verbosity, bool score_only, bool local_only, tee& log, const terms& t, const flv& weights, tee& result) {
    ...
    if(score_only) {
        ...
        VINA_FOR_IN(i, out_cont) {
            ...
            /*result << "  " << std::setw(9) << std::setprecision(3) << lb
                   << "  " << std::setw(9) << std::setprecision(3) << ub; // oxis*/
        ...
        doing(verbosity, "Writing output", log);
        //write_all_output(m, out_cont, how_many, out_name, remarks); oxis
        done(verbosity, log);
...
void main_procedure(model& m, const boost::optional<model>& ref, // m is non-const (FIXME?)
                 ...
                 const flv& weights,
                 int cpu, int seed, int verbosity, sz num_modes, fl energy_range, tee& log, tee& result) {
    ...
    if(randomize_only) {
        ...
        if(no_cache) {
            do_search(m, ref, wt, prec, nc, prec_widened, nc_widened, nc,
                      ...
                      seed, verbosity, score_only, local_only, log, t, weights, result);
        }
        else {
...
#################################################################\n";
    ...
    try {
        std::string rigid_name, ligand_name, flex_name, config_name, out_name, log_name, real_ligand, real_receptor; //oxis edited
        ...
        bool score_only = false, local_only = false, randomize_only = false, help = false, help_advanced = false, version = false, test=false; // FIXME //oxis
        ...
        outputs.add_options()
            ...
            ("rligand", value<std::string>(&real_ligand), "optionally, real filename for ligand, boinc application") //oxis, extract the real names
            ("rreceptor", value<std::string>(&real_receptor), "optionally, real filename for receptor, boinc application") //oxis
            ("test", bool_switch(&test), "optionally, generate a valid result and quit") //oxis
        ;
        ...
        tee log;
        tee result;//oxis: output just the result
        result.init("result.log");//oxis: output just the result

        if(vm.count("rreceptor") > 0) {
        	result << real_receptor << ",";
        	std::cerr << "\nReceptor name: " << real_receptor << '\n';
        }
        if(vm.count("rligand") > 0) {
        	result << real_ligand << ",";
        	std::cerr << "\nLigand name: " << real_ligand << '\n';
    	}
        ...
        main_procedure(m, ref,
                    ...
                    gd, exhaustiveness,
                    weights,
                    cpu, seed, verbosity, max_modes_sz, energy_range, log, result);
